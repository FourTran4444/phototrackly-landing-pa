...
export default function LeadForm({ variant }: { variant: 'hero' | 'footer' }) {
  ...
  // POST is explicit even before hydration; never put contact details in a URL.
  return <form ref={form} method="post" action="/api/early-access" onSubmit={submit} onFocusCapture={start} className={`pl-form pl-form-${variant}`} aria-label={full ? 'Early-access registration' : 'Quick early-access registration'} aria-busy={pending}>
